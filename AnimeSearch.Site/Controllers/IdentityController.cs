...
public class IdentityController: BaseController
{
    ...
    public IdentityController(AsmsearchContext database, UserManager<Users> userManager, SignInManager<Users> signInManager, RecaptchaService recaptchaService) : base(database)
    {
    ...
    public async Task<ActionResult> RegisterGet(string returnUrl = null)
    {
        ...
        ViewData["recaptchaSiteKey"] = _database.Settings.GetValueOrDefault(DataUtils.SettingRecaptchaSiteKey);
    ...
    public async Task<ActionResult> RegisterPost([Bind] InputRegisterModel InputRegister, string returnUrl = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(_database.Settings.GetValueOrDefault(DataUtils.SettingRecaptchaSiteKey) ?? string.Empty))
        {
            if (!await _recaptchaService.IsValid(InputRegister.RecaptchaToken))
            {
                ModelState.AddModelError(string.Empty, "recaptcha invalide. Seriez-vous un robot ?");
                ViewData["recaptchaSiteKey"] = _database.Settings.GetValueOrDefault(DataUtils.SettingRecaptchaSiteKey);
                return View("register");
            }
        }
        ...
        if (ModelState.IsValid)
        {
            ...
            if (InputRegister.UserName == DataUtils.Guest)
            {
