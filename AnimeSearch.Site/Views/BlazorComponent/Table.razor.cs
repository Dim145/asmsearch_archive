...
public partial class Table<TableItem> : ITable<TableItem>
{
    ...
    [Parameter]
    public bool DefaultLoadingInProgress { get; set; } = false;
    ...
    protected override async Task OnParametersSetAsync()
    {
        ...
        LoadingInProgress = DefaultLoadingInProgress;
    }
    ...
    private readonly Dictionary<int, bool> detailsViewOpen = new();
    ...
    private async Task LoadServerSideDataAsync()
    {
        if (DataLoader != null)
        {
            ...
            {
                var itemsq = result.Records.AsQueryable();

                foreach (var item in Columns)
                {
                    if (item.Filter != null)
                    {
                        itemsq = itemsq.Where(item.Filter);
                    }
                }

                Items = itemsq.Skip(result.Skip).Take(result.Top);
    ...
    {
        page = Math.Max(Math.Min(page, TotalPages), 0);
    ...
    [Parameter]
    public Action<TableItem, MouseEventArgs> RightClickAction { get; set; }

    /// <summary>
    /// Handles the onclick action for table rows.
    /// This allows the RowClickAction to be optional.
    /// </summary>
    private void OnRowClickHandler(TableItem tableItem, MouseEventArgs e)
    {
        try
        {
            if(e.Button == 2)
                RightClickAction?.Invoke(tableItem, e);
            else
    ...
    private Expression<Func<TableItem, bool>> GlobalSearchQuery(string value)
    {
        ...
        foreach (string keyword in value.Trim().Split(" "))
        {
            ...
            {
                var newQuery = System.Linq.Expressions.Expression.Lambda<Func<TableItem, bool>>(
                    System.Linq.Expressions.Expression.AndAlso(
                        column.Field.Body.CreateNullChecks(),
                        System.Linq.Expressions.Expression.GreaterThanOrEqual(
                            System.Linq.Expressions.Expression.Call(
                                System.Linq.Expressions.Expression.Call(column.Field.Body, "ToString", Type.EmptyTypes),
                                typeof(string).GetMethod(nameof(string.IndexOf), new[] { typeof(string), typeof(StringComparison) }),
                                new[] { System.Linq.Expressions.Expression.Constant(keyword), System.Linq.Expressions.Expression.Constant(StringComparison.OrdinalIgnoreCase) }),
