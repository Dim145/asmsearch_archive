...
namespace AnimeSearch.Services
{
    public class TelegramService : BaseService
    {
        ...
        public TelegramService(AsmsearchContext database) : base("Bot Télégram", TimeSpan.FromHours(1), "Contrôle le bot permettant d'éxécuter des recherches directement depuis Télégram.")
        {
            _database = database;
        ...
        public override Task ExecutionCode()
        {
            if (!IsRunning && (cts == null || cts.IsCancellationRequested))
            {
                try
                {
                    var setting = _database.Settings.AsNoTracking().FirstOrDefaultAsync(s => s.Name == SETTING_TOKEN_TELEGRAM_NAME).GetAwaiter().GetResult();
                    ...
                    SetTelegramLink(me.Username);
                }
                catch (Exception ex)
                {
                    AddExceptionError("le redémarrage du bot Télégram", ex);
        ...
        {
            AddExceptionError("bot télégram", exception);
        ...
        private async Task Search(string search, Users databaseUser, long chatId)
        {
            ...
            if (res != null && res.Result.GetUrl() != null && res.SearchResults.Values.Any(m => m.NbResults > 0))
            {
        ...
        private async Task Infos(string search, Users databaseUser, long chatId)
        {
            ...
            if (res.GetUrl() != null)
            {
                string response = $"Résultat: {Regex.Replace(res.GetName(), @"([()!\-|\\*\.=+])", @"\$1")}\n";                    

                if (genres?.Length > 0)
                   response += $"genres: {Regex.Replace(string.Join(", ", genres), @"([()!\-|\\*\.=+])", @"\$1")}\n";

                response += $"Date de sortie: {res.GetRealeaseDate().GetValueOrDefault():dd/MM/yyyy}\n";
                ...
                if (res.GetImage() != null)
                    await client.SendPhotoAsync(chatId, photo: new(res.GetImage()), caption: response, parseMode: ParseMode.MarkdownV2);
                else
                    await client.SendTextMessageAsync(chatId, response, ParseMode.MarkdownV2);
            }
            else
        ...
        private async Task Msearch(string search, Users databaseUser, long chatId)
        {
            ...
            var res = SiteSearch.MultiSearch(search);
            ...
            else
            {
                ...
                foreach (ModelMultiSearch model in res)
                {
                    ...
                    string url  = new Uri(BASE_URL + model.Lien).GetComponents(UriComponents.HttpRequestUrl, UriFormat.UriEscaped);
                    url = Regex.Replace(url, @"([()!\-|\\*\.=+])", @"\$1");
        ...
        private void ExecCommandByName(BotCommands command, string search, Users databaseUser, long chatId)
        {
            ...
            if (method != null)
            {
                var res = method.Invoke(this, Tab(search, databaseUser, chatId));
