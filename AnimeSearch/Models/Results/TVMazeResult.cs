...
namespace AnimeSearch.Models.Results
{
    ...
    {
        private Dictionary<string, Uri> images = null;
        ...
        public float Score { get => GetPopularity(); set {
                if(popularity == null)
                    SetPopularity(value);
            } 
        }

        public Dictionary<string, float?> Rating { get => null; set 
            {
                if (value != null && value.GetValueOrDefault("average") != null)
                    SetPopularity(value["average"].Value);
            } }
