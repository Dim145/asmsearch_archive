...
namespace AnimeSearch.Controllers.api
{
    ...
    public class ApiController : Controller
    {
        ...
        public ApiController(AsmsearchContext database, MailService ms, UserManager<Users> userManager)
        {
            _database = database;
        ...
        public async Task<ActionResult<ModelAPI>> SearchSerieAsync(string search)
        {
            ...
            var res = await Task.Run(() => SiteSearch.Search("serie=>" + search, HttpContext.Request.Cookies));

            if(res.Result.GetUrl() != null)
                await Utilities.CreateOrUpdateSearch(User?.Identity?.Name, res.Result.GetName(), _database);
        ...
        public async Task<ActionResult<ModelAPI>> SearchAsync(string search)
        {
            ...
            var res = await Task.Run(() => SiteSearch.Search(search, HttpContext.Request.Cookies));
        ...
        public async Task<ActionResult<ModelAPI>> SearchFilmAsync(string search)
        {
            ...
            var res = await Task.Run(() => SiteSearch.Search("movie" + search, HttpContext.Request.Cookies));
        ...
        public async Task<ActionResult<ModelMultiSearch[]>> MSearchAsync(string search)
        {
            if (string.IsNullOrWhiteSpace(search))
                return BadRequest("Valeur Null");

            return Ok(await Task.Run(() => SiteSearch.MultiSearch(search)));
        }
        ...
        public async Task<OkObjectResult> GetSerieAsync(string search) => Ok(await GetSerie(search));
        ...
        public async Task<OkObjectResult> GetFilmsAsync(string search) => Ok(await GetFilms(search));
        ...
        [HttpGet("film/{search}")]
        public async Task<OkObjectResult> GetFilmAsync(string search) => Ok(await GetFilm(search));
        ...
        public async Task<IActionResult> TestUrlAsync(string url)
        {
            ...
            return await OtherUtils.TestUrl(url) ? Ok() : NotFound();
        ...
        public async Task<ActionResult<string>> GetHtmlSiteAsync(string url, [FromForm] Dictionary<string, string> postValues)
        {
            ...
            var response = await (postValues == null || postValues.Count == 0 ? Client.GetAsync(url) : Client.PostAsync(url, new FormUrlEncodedContent(postValues)));
            ...
            int indexStart = html.IndexOf(Utilities.BALISE_SCRIPT_DEBUT, StringComparison.InvariantCulture); // enleve les scripts
            while(indexStart > -1)
            {
                int indexEnd = html.IndexOf(Utilities.BALISE_SCRIPT_FIN, StringComparison.InvariantCulture) + Utilities.BALISE_SCRIPT_FIN.Length;

                html = string.Concat(html.AsSpan(0, indexStart), html.AsSpan()[indexEnd..]);
                indexStart = html.IndexOf(Utilities.BALISE_SCRIPT_DEBUT, StringComparison.InvariantCulture);
            }

            indexStart = html.IndexOf(Utilities.BALISE_LINK_DEBUT, StringComparison.InvariantCulture); // enleve le css
            while(indexStart > -1)
            {
                int indexEnd = html.IndexOf(Utilities.BALISE_LINK_FIN, indexStart, StringComparison.InvariantCulture) + Utilities.BALISE_LINK_FIN.Length;

                html = string.Concat(html.AsSpan(0, indexStart), html.AsSpan()[indexEnd..]);
                indexStart = html.IndexOf(Utilities.BALISE_LINK_DEBUT, StringComparison.InvariantCulture);
            }
        ...
        public async Task<IActionResult> AddSite([FromForm] Sites site)
        {
            ...
            if (!await OtherUtils.TestUrl(site.Url))
        ...
        public async Task<IActionResult> MakeDonation(string amount, int type)
        {
            ...
            if(type == 0) // PayPal
            {
                try
                {
                    var email = (await _database.Settings.FirstOrDefaultAsync(s => s.Name == Utilities.SETTING_PAYPAL_MAIL_NAME))?.GetValueObject<string>();
            ...
            else if(type == 1) // BTC
            {
                ...
                using var request = new HttpRequestMessage(HttpMethod.Post, Utilities.OPENNODE_API_URL);
                request.Headers.Add("Authorization", (await _database.Settings.FirstOrDefaultAsync(s => s.Name == Utilities.SETTING_OPENNODE_APIKEY))?.GetValueObject());
                ...
                try
                {
                    ...
                    {
                        Utilities.AddExceptionError("la réponse de l'api d'Open Node", new Exception(await response.Content.ReadAsStringAsync()));
                ...
                {
                    Utilities.AddExceptionError($"Dans l'envoi de la requete à OpenNode ({Utilities.OPENNODE_API_URL}) avec les données: '{await request.Content.ReadAsStringAsync()}'", ex);
        ...
        public async Task<IActionResult> Dons_doneAsync() // méthode appelé automatiquement par le callback de paypal. => ne peut etre en patch
        {
            try
            {
                ...
                if(idExist)
                {
                    ...
                    string mail = (await _database.Settings.FirstOrDefaultAsync(s => s.Name == Utilities.SETTING_PAYPAL_MAIL_NAME))?.GetValueObject();
            ...
            if(callback != null && Guid.Empty != callback.Id)
            {
                ...
                if (d == null)
                {
                    d = new()
                    {
                        ...
                        User_id = (await _userManager.FindByNameAsync(Utilities.GUEST)).Id,
                        Done = false
                    };
        ...
        {
            return Ok(await DatabaseCom.GetRecherchesDatas(_database));
        }
        ...
        public async Task<IActionResult> SaveRechercheResult([FromForm] ModelAPI model)
        {
            ...
            return Ok(await DatabaseCom.SaveRechercheResult(_database, User.Identity?.Name, model));
        }
        ...
        public async Task<ActionResult<List<SavedSearch>>> GetSavedSearch()
        {
            ...
            return Ok(await DatabaseCom.GetAllSavedSearchs(_database, user.Id));
        }
        ...
        public async Task<ActionResult<SavedSearch>> GetSavedSearch(string search)
        {
            ...
            return Ok(await DatabaseCom.GetSavedSearch(_database, search, user.Id));
        }
        ...
        private async Task<ActionResult<bool>> DeleteSave(string search)
        {
            ...
            return Ok(await DatabaseCom.DeleteSave(_database, search, user.Id) > 0);
        }

        [HttpPost("advance_search")]
        public async Task<IActionResult> AdvanceSearchs([FromForm] AdvanceSearch advanceSearch)
        {
            return Ok(await RechercheByGenresAsync(advanceSearch));
        }
    }
}
